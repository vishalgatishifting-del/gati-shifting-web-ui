...
type Category = "all" | "bike" | "household" | "wooden" | "storage" | "car";
...
const CATEGORY_META: CategoryMeta[] = [
  ...
  { key: "bike",      label: "Bike Shifting",  emoji: "🏍️"  },
  { key: "household", label: "House Shifting", emoji: "🏠"  },
  { key: "storage",   label: "Storage",        emoji: "📦"  },
  { key: "car",       label: "Car Shifting",   emoji: "🚗"  },
  { key: "wooden",    label: "Wooden Packing", emoji: "🪵"  },
];
...
// ─── GalleryImage ─────────────────────────────────────────────────────────────
// Memoized to prevent unnecessary re-renders during category filter changes
...
// ─── Total image count for badge display ─────────────────────────────────────

const TOTAL_COUNTS: Record<string, number> = Object.fromEntries(
  Object.entries(IMAGES).map(([k, v]) => [k, v.length])
);
TOTAL_COUNTS["all"] = Object.values(IMAGES).reduce((s, a) => s + a.length, 0);
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content="Browse photos of our packing, loading, and transportation processes. Gati Shifting Packers demonstrates safe and professional handling for every relocation."
        />
        <meta
          name="keywords"
          content="Gati Shifting Packers photo gallery, packing and moving photos, relocation services images, professional movers photos, client success stories"
        />
        ...
        <meta
          ...
          content="Browse the Photo Gallery of Gati Shifting Packers to see our expert packing and moving services in action."
        />
        ...
        <meta
          ...
          content="Check out our Photo Gallery showcasing professional packing, moving, and relocation services."
        />
        ...
        <link rel="canonical" href="https://gatishiftingpackers.com/photo-gallery" />
      </Helmet>

      {/* ── Banner ── */}
      <div className="pg-banner">
        <div className="pg-banner__bg" aria-hidden="true" />
        <div className="pg-banner__content">
          <span className="pg-tag">Our Work</span>
          <h1>Photo Gallery</h1>
          <p>A closer look at how we handle every move — with care, precision, and professionalism.</p>
        </div>
      </div>
      ...
      <section className="pg-section">
        <div className="pg-container">
          ...
          {/* Filter Bar */}
          <div className="pg-filters" role="group" aria-label="Filter photos by category">
            {CATEGORY_META.map(({ key, label, emoji }) => (
              <button
                key={key}
                className={`pg-filter-btn ${category === key ? "active" : ""}`}
                onClick={() => handleCategory(key)}
                aria-pressed={category === key}
              >
                <span className="filter-emoji" aria-hidden="true">{emoji}</span>
                <span className="filter-label">{label}</span>
                <span className="filter-count">{TOTAL_COUNTS[key]}</span>
              </button>
            ))}
          </div>
